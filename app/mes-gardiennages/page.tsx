  ...
  useEffect(() => {
    ...
    const fetchPlantes = async () => {
      try {
        ...
        const responseGardiennages = await fetch(`http://15.237.67.223:3000/api/gardiennage/afficherGardes?idUtilisateur=${idUtilisateur}`, { headers });
        ...
        const plantesPromises = idsPlantes.map(async (idPlante: number) => {
          try {
            const responsePlante = await fetch(`http://15.237.67.223:3000/api/plante/afficher?idPlante=${idPlante}`, { headers });
