...
const PlantDetailPage: React.FC<{ params: { id: string } }> = ({ params }) => {
    ...
    useEffect(() => {
        const fetchPlante = async () => {
            try {
                ...
                const response = await fetch(`http://15.237.67.223:3000/api/plante/afficher?idPlante=${id}`, { headers: headers });
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await axios.post('http://15.237.67.223:3000/api/conseil/ajouter', null, {
                params: {
                    description: conseil,
                    idPlante: id,
