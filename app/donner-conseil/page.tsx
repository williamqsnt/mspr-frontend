...
export default function PlanteDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserId = async () => {
      try {
        if (pseudo) {
          const response = await fetch(`http://15.237.67.223:3000/api/utilisateur/recupererId?pseudo=${pseudo}`, {
            headers: {
  ...
  const fetchPlanteDetails = async (id: string) => {
    try {
      ...
      const response = await fetch(`http://15.237.67.223:3000/api/plante/afficher?idPlante=${id}`, {
        headers: {
  ...
  const handleConseilSubmit = async () => {
    ...
    try {
      ...
      const url = new URL('http://15.237.67.223:3000/api/conseil/ajouter');
